      ...
      <div className="max-w-3xl mx-auto bg-white rounded-2xl shadow-glass print:shadow-none print:rounded-none p-8 sm:p-12 mb-10 print:mb-0 print:max-w-full">
        ...
        <div className="grid sm:grid-cols-2 gap-6 mb-6 text-sm">
          ...
          <div>
            ...
            {order.fulfillment_type === 'delivery' && (
              order.contact_phone
                ? <p className="text-ink/50">{order.contact_name || 'No name given'} — {order.contact_phone}</p>
                : order.shipping_address && <p className="text-ink/50 whitespace-pre-line">{order.shipping_address}</p>
            )}
        ...
        {(type === 'packing_slip' || type === 'delivery_note') && (
          <div className="bg-surface/60 rounded-xl2 p-4 mb-6 text-sm text-ink/70">
            ...
            {order.contact_phone ? (
              <p>{order.contact_name || 'No name given'} — <a href={`tel:${order.contact_phone}`} className="text-secondary underline">{order.contact_phone}</a></p>
            ) : (
